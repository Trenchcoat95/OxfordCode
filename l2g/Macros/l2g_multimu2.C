...
void l2g_multimu()
{
    ...
    TVector3 LArDim(714.7,254.55,150.51);
    ...
    TH2F* N2_DTSpread = new TH2F("N2_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);
    TH2F* N3_DTSpread = new TH2F("N3_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);
    TH2F* N4_DTSpread = new TH2F("N4_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);

    TH2F* PN2_DTSpread = new TH2F("PN2_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);
    TH2F* PN3_DTSpread = new TH2F("PN3_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);
    TH2F* PN4_DTSpread = new TH2F("PN4_DTSpread", "DTSpread at LAr Exit", 100, 0, 100, 100, 0, 800);
    ...
    for (Int_t i=0; i<nentries; i++) 
    {
        ...
        for (Int_t j=0; j<xdm.size(); j++)
        {
            for (Int_t k=0; k<=j; k++)
            {
              if(j!=k) dx.push_back(abs(xdm.at(k)-xdm.at(j)));
              if(j!=k) dy.push_back(abs(ydm.at(k)-ydm.at(j)));
              if(j!=k) dt.push_back(abs(tdm.at(k)-tdm.at(j)));
