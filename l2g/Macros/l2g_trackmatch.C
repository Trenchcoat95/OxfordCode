...
void l2g_trackmatch()
{
    ...
    TH2F* nhitsVSfrac_resid = new TH2F("nhitsVSfrac_resid", "Fractional residuals",20,0,20, 50, -0.4, 0.4);
    TH2F* PnhitsVSfrac_resid = new TH2F("PnhitsVSfrac_resid", "P(Residual|nhits)",20,0,20, 50, -0.4, 0.4);
        ...
        for (Int_t j=0; j<PDG->size(); j++) 
        {
           if(PDG->at(j)==13 || PDG->at(j)==-13)
           {
               ...
               if(InGAr!=0)
               {
                    ...
                    hz->Fill(vecX.Z());
                    hcos->Fill(cosTmax);
                    if(cosTmax>0.995)hdelX->Fill(delX);
                    ...
                    if (whichEnd==0)
                    {
                        ...
                        if(p<=2.5 && p>=2.3&&(preco-p)/p<0.1)weird.push_back(i);
                        //if(p<=2.5 && p>=2.3) 
                        //if(p<=2.5 && p>=2.3)
                        ...
                        if(p<=2.5 && p>=2.3)nhitsVSfrac_resid->Fill(NTPCClustersOnTrack->at(iRECOpart),(preco-p)/p);
                        if(p<=2.5 && p>=2.3)PnhitsVSfrac_resid->Fill(NTPCClustersOnTrack->at(iRECOpart),(preco-p)/p);
    ...
    for (Int_t x=1; x<=20; x++)
    {
        ...
        for (Int_t y=1; y<=50; y++)
        {
            if (max<PnhitsVSfrac_resid->GetBinContent(x,y))  max=PnhitsVSfrac_resid->GetBinContent(x,y);
        }
        
        for (Int_t y=1; y<=50; y++)
        {
            float bin=0;
            if (max!=0) bin=PnhitsVSfrac_resid->GetBinContent(x,y)/max;
            PnhitsVSfrac_resid->SetBinContent(x,y,bin);
    ...
    for(int t=0; t<weird.size(); t++)
    {
        std::cout<<weird.at(t)<<std::endl;
    }
    ...
    hnhitsmatched->Draw();
    //mccanvasnhitsmatched->Print("nhitsmatched.png");
