        ...
        for (Int_t j=0; j<PDG->size(); j++) 
        {
           if(PDG->at(j)==13 || PDG->at(j)==-13)
           {
               ...
               if(InGAr!=0)
               {
                    ...
                    for (int i=0; i<trajinGAr.size(); i++)
                    {
                        px=TrajMCPPX->at(trajinGAr.at(i));
                        py=TrajMCPPY->at(trajinGAr.at(i));
                        pz=TrajMCPPZ->at(trajinGAr.at(i));
                        p = sqrt(px*px+py*py+pz*pz);
                        x=TrajMCPX->at(trajinGAr.at(i));
                        y=TrajMCPY->at(trajinGAr.at(i));
                        z=TrajMCPZ->at(trajinGAr.at(i));
                        TVector3 MCpartTemp(px,py,pz);
                        TVector3 MCpartHatTemp=MCpartTemp.Unit();
                    for (int iTrack=0; iTrack<nTracks; ++iTrack) 
                    {
                            ...
                            // Direction matching
                            double cosTbeg = MCpartHatTemp.Dot(RECO_P_beg)/RECO_P_beg.Mag();
                            double cosTend = MCpartHatTemp.Dot(RECO_P_end)/RECO_P_end.Mag();

                            if(i==0)hnhits->Fill(NTPCClustersOnTrack->at(iTrack));
