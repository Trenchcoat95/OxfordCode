    ...
    namespace rec {
        ...
        if (true)
        {
            ...
            for(size_t t=0; t<TPCClusterColTime.size(); t++)
                ...
                outfile.open("time_timedivision_planes.txt", std::ios_base::app); // append instead of overwrite
                outfile << ntpcclust << "  " << timer.RealTime() << "\n"; 
                }
                timer0.Stop();

                std::ofstream outfile0;

                outfile0.open("time_timedivision_planes0.txt", std::ios_base::app); // append instead of overwrite
                outfile0 << ntpcclus << "  " << timer0.RealTime() << "\n"; 
            }   
