...
namespace gar {
    namespace rec {
        ...
        if (true)
        {
            ...
            std::list<size_t> unusedTPCplane1;
            std::list<size_t> unusedTPCplane2;
            std::list<size_t> unusedTPCplane3;
            //std::copy(TPCClusterCol->begin(), TPCClusterCol->end(), std::back_inserter(unusedTPC));
            float firstpos=-1.;
            for(size_t i=0; i<ntpcclus; i++)
            {
                unusedTPC.push_back(i);
                if(TPCClusterCol->at(i).Position()[2]-TPCClusterCol->at(0).Position()[2]<=4) unusedTPCplane1.push_back(i);
                if(TPCClusterCol->at(i).Position()[2]-TPCClusterCol->at(0).Position()[2]>=4) 
                {
                    unusedTPCplane2.push_back(i);
                    if (firstpos<0.) 
                    {
                    firstpos=TPCClusterCol->at(i).Position()[2];
                    std::cout<<"firstpos="<<firstpos<<std::endl;
                    }
                }
                if(TPCClusterCol->at(i).Position()[2]-firstpos>=4 && firstpos>0) unusedTPCplane3.push_back(i);
            }
            ...
            std::cout << "};\n";
            std::cout << "l1 = { ";
            for (size_t n : unusedTPCplane1) {
            std::cout << TPCClusterCol->at(n).Position()[2] << ", ";
            }
            std::cout << "};\n";
            std::cout << "l2 = { ";
            for (size_t n : unusedTPCplane2) {
            std::cout << TPCClusterCol->at(n).Position()[2] << ", ";
            }
            std::cout << "};\n";
            std::cout << "l3 = { ";
            for (size_t n : unusedTPCplane3) {
            std::cout << TPCClusterCol->at(n).Position()[2] << ", ";
            }
            std::cout << "};\n";
            int done= 0;
