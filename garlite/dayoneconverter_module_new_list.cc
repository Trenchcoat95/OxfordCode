...
namespace gar {
    namespace rec {
        ...
        if (true)
        {
            ...
            std::list<gar::rec::TPCCluster> unusedTPC;
            std::copy(TPCClusterCol->begin(), TPCClusterCol->end(), std::back_inserter(unusedTPC));
            std::list<gar::rec::TPCCluster>::iterator it;
            std::list<gar::rec::TPCCluster>::iterator jt;
            std::list<gar::rec::TPCCluster>::iterator kt;
            std::list<gar::rec::TPCCluster>::iterator kt2;
            int done= 0;
            //int besti=0;
            //int bestj=0;
            //int bestk=0;
            ...
            while (done==0)
            {
                    ...
                    for(jt = ++it; jt != unusedTPC.end(); ++jt)
                    {
                        ...
                        for (kt= ++jt; kt != unusedTPC.end(); ++kt)
                        {
                            ...
                            for(kt2 = unusedTPC.begin(); kt2 != unusedTPC.end(); ++kt2)
                            {
                                ...
                                if (kt2 == unusedTPC.end().prev() && tpcclusindexb > -1)
                                {
                                    //tpcclusindex.push_back(tpcclusindexb);
                                    TPCtrial.push_back(tpcclusb);
                                    sumr2 += dbest*dbest;
                            ...
                            if (TPCtrial.size() > bestnpts ||
                            ((TPCtrial.size() == bestnpts) &&
                            (sumr2 < bestsumr2)))
                            {
                                bestnpts = TPCtrial.size();
                                bestsumr2 = sumr2;
                                tcv=TPCtrial;
                ...
                */
                ...
                {
                   auto l = std::find(unusedTPC.begin(), unusedTPC.end(), tcv.at(i));
                   unusedTPC.erase(l);
                }
