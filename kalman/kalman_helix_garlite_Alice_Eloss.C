      ...
      for (size_t iTPCCluster=1; iTPCCluster<xyz_plane.size(); ++iTPCCluster)
        {
          ...
          */
          ...
          //PPred[4][4]/=0.5*0.299792458e-2;
          //PPred[4][2]/=0.5*0.299792458e-2;
          //PPred[4][0]/=0.5*0.299792458e-2;
...
void kalman_helix_garlite_Alice_Eloss(size_t nevents)

    {
      ...
      TFile fs("garlitetest_x1y1smear_realseed_elossnocorr.root","recreate");
                ...
                (xyzprev.X()>Plane_XY[0] && xyzprev.X()<Plane_XY[1] && xyzprev.Y()>Plane_XY[2] && xyzprev.Y()<Plane_XY[3]))
              {
                if((xyztemp.Z()>Planes_Z[p*2] && xyztemp.Z()<Planes_Z[p*2+1])||
                   (xyzprev.Z()>Planes_Z[p*2] && xyzprev.Z()<Planes_Z[p*2+1]))
                  {
                    ...
                    status= CorrectForMeanMaterial(-deltaxyz*rho,muon_mass,0.005,sqrt(pxyztemp.Mag2()),charge,(sqrt(pxyztemp.Mag2())/muon_mass),rho,X0,X1,I,ZA,invpTtemp);
            ...
            for(size_t p=0;p<5;p++)
            {
              ...
              if(xyztemp.X()>Plane_XY[0] && xyztemp.X()<Plane_XY[1] && xyztemp.Y()>Plane_XY[2] && xyztemp.Y()<Plane_XY[3])
              {
                if(xyztemp.Z()>Planes_Z[p*2] && xyztemp.Z()<Planes_Z[p*2+1] && InPlane[p]==0)
                  {
                    InPlane[p]=1;
                    int hitsinplane=1+gRandom->Gaus(3,1);
                    ...
                    double tanlambda_plane_temp=tanlambdatemp;
                    ...
                    for(int i=0;i<hitsinplane;i++)
                      {
                        ...
                        xyz_plane_temp.SetX(xyz_plane_temp.X()+tanlambda_plane_temp/curvature_plane_temp*rotp);

                        xyz_plane_temp_sm.SetX(gRandom->Gaus(xyz_plane_temp.X(),1));
                        xyz_plane_temp_sm.SetY(gRandom->Gaus(xyz_plane_temp.Y(),1));
                        xyz_plane_temp_sm.SetZ(xyz_plane_temp.Z());
                        ...
                        tanlambda_plane.push_back(tanlambda_plane_temp);
                        invpT_plane.push_back(curvature_plane_temp/(0.5*0.299792458e-2));
      ...
      */
      ...
      //std::cout<<parvect_bkw.size()<<std::endl;
