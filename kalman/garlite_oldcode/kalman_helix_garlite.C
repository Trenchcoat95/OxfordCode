...
//--------------------------------------------------------------------------------------------------------------
                                ...
                                )
    {
      ...
      /*
      double curvature_init= 0.001336709363386035;         ///need to check what this is
      double phi_init = 2.415579080581665;
      double lambda_init = 0.037335269153118134;
      double xpos_init= -23.076112747192383;
      double ypos_init=-340.07403564453125;
      double zpos_init=1638.2122802734375;
      */
      ...
      if(fPrintLevel>0)
      {
        std::cout<<std::endl<<"Seed parameters: "<<xyz_seed.X()<<" "<<xyz_seed.Y()<<" "<<xyz_seed.Z()<<" "<<curvature_seed<<" "<<phi_seed<<" "<<lambda_seed<<std::endl<<std::endl;
      }
      ...
      */
      ...
      double fTPCClusterResolX=0.5;//0.5;
      ...
      for (size_t iTPCCluster=1; iTPCCluster<xyz_plane.size(); ++iTPCCluster)
        {
          ...
          /*
            double dxnum = (slope/(fTPCClusterResolYZ*fTPCClusterResolYZ))*( (yh - parvec[0])*TMath::Sin(phi) + (zh - parvec[1])*TMath::Cos(phi) )
            + (xh - xpos)/(fTPCClusterResolX*fTPCClusterResolX);
            double dxdenom = slope*slope/(fTPCClusterResolYZ*fTPCClusterResolYZ) + 1.0/(fTPCClusterResolX*fTPCClusterResolX);
            dx = dxnum/dxdenom;
            if (dx == 0) dx = 1E-3;
          */
          ...
          predstep[0] += dir*slope*dx*TMath::Sin(phi);  // update y
          ...
          double yprev = parvec[0];
          double zprev = parvec[1];
...

    {
      ...
      std::vector<double> xht,yht,zht,xpost;
      ...
      std::vector<double> xht_bkw,yht_bkw,zht_bkw,xpost_bkw;
      ...
      t1s.Branch("xpost_bkw",&xpost_bkw);
      ...
      double curvaturetemp =charge*0.01*(0.3*B)/ptotal; //in cm^-1
                    ...
                    for(int i=0;i<hitsinplane;i++)
                      {
                        ...
                        double dz=Plane_thick/(hitsinplane+1);

                        xyz_plane_temp.SetX(xyz_plane_temp.X()+dz/(slope_plane_temp*TMath::Cos(phi_plane_temp)));
                        xyz_plane_temp.SetY(xyz_plane_temp.Y()+dz*TMath::Sin(phi_plane_temp)/TMath::Cos(phi_plane_temp));
                        xyz_plane_temp.SetZ(xyz_plane_temp.Z()+dz);
                        phi_plane_temp+=dz*curvature_plane_temp/TMath::Cos(phi_plane_temp);
                        ...
                        XYZVector hit=xyz_plane_temp;
                        xyz_plane.push_back(hit);
                        xyz_plane_m.push_back(hit);
      ...
      */
      if(xyz_plane.size()>0 && nHits_perPlane.size()>=3) 
      {
        ...
        */
        ...
        KalmanFit(xht,yht,zht,parvect,predstept,Pt,PPredt,Rt,xpost,xyz_plane,Ry,Rz,Ryz,xyz_seed,lambda_seed,curvature_seed,phi_seed,forward);
        ...
        double backwards=-1.;
        KalmanFit(xht_bkw,yht_bkw,zht_bkw,parvect_bkw,predstept_bkw,Pt_bkw,PPredt_bkw,Rt_bkw,xpost_bkw,xyz_plane_bkw,Ry,Rz,Ryz,xyz_seed_bkw,lambda_seed_bkw,curvature_seed_bkw,phi_seed_bkw,backwards);
      }
      t1s.Fill();
