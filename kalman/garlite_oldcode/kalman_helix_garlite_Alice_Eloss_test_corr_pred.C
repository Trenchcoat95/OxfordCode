              ...
              if((zh>Planes_Z[p*2] && zh<Planes_Z[p*2+1]))
                  {
                    ...
                    Bool_t checkstatus = Propagate(dz,PPred, P, predstep,  parvec, kAlmost0, kAlmost1, fPrintLevel, dir, InPlane, dErec, dxyzrec, Energy_loss);
                      if (!checkstatus)
                      { 
                        status=checkstatus;
                        break;
              ...
              if((zh>Planes_Z[p*2] && zh<Planes_Z[p*2+1]))
                  {
                    ...
                    InPlane=1;
                    Bool_t checkstatus = Propagate(dzmid,PPred, P, predstep,  parvec, kAlmost0, kAlmost1, fPrintLevel, dir, InPlane, dErec, dxyzrec, Energy_loss);
                    dErectot+=dErec;
                    dxyzrectot+=dxyzrec;
                    if (!checkstatus)
                    { 
                    ...
                    checkstatus = Propagate(dzmid,PPred, Predmidstep, predstep,  predmidstep, kAlmost0, kAlmost1, fPrintLevel, dir, InPlane, dErec, dxyzrec, Energy_loss);
                    if (!checkstatus)
                    { 
                      status=checkstatus;
                      break;
...
void kalman_helix_garlite_Alice_Eloss_test_corr_pred(size_t nevents)

    {
      ...
      TFile fs("./toygarlite/nosmear_perfectseed_noelossnocorr/garlitetest_nosmear_perfectseed_noelossnocorr_r20_20.root","recreate");
      //TFile fs("test.root","recreate");
      TTree t1s("t1s","helix simple tree");

      ///Parameters regulating simulation
      std::string Seedtype = "perfect";
      std::string Energy_Smear = "no";
      Bool_t Energy_loss_corr = false;
      Bool_t Energy_loss = false;
      Bool_t Smear = false;
      ...
      */
      ...
      if(xyz_plane.size()>0 && nHits_perPlane.size()>=3 && status==true) 
      {
        ...
        if(Smear==kFALSE) KalmanFit(xht,yht,zht,parvect,predstept,Pt,PPredt,Rt,zpost,xyz_plane,sinphi_plane,Ry,Rx,Ryx,xyz_seed,tanlambda_seed,curvature_seed,sinphi_seed,forward,status,printlevelKalman,dEreco,dxreco,Energy_loss_corr);
        else KalmanFit(xht,yht,zht,parvect,predstept,Pt,PPredt,Rt,zpost,xyz_plane_sm,sinphi_plane,Ry,Rx,Ryx,xyz_seed,tanlambda_seed,curvature_seed,sinphi_seed,forward,status,printlevelKalman,dEreco,dxreco,Energy_loss_corr);
        ...
        KalmanFit(xht_bkw,yht_bkw,zht_bkw,parvect_bkw,predstept_bkw,Pt_bkw,PPredt_bkw,Rt_bkw,zpost_bkw,xyz_plane_bkw,sinphi_plane_bkw,Ry,Rx,Ryx,xyz_seed_bkw,tanlambda_seed_bkw,curvature_seed_bkw,sinphi_seed_bkw,backwards,status,printlevelKalman,dEreco,dxreco,Energy_loss_corr);
        //std::cout<<"status= "<<status<<std::endl;
        }
