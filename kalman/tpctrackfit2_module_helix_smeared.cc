...
namespace gar {
  namespace rec {
      ...
      else
	{
      ...
      TFile f("/dune/data/users/battisti/thinecaltest/Energy_samples/smeared_helix.root","update");
      ...
      for (size_t iTPCCluster=1; iTPCCluster<nTPCClusters; ++iTPCCluster)
        {
          ...
          if (iTPCCluster>1)
          {
            float dx = rnd->Gaus(0.04,0.01);
            xh+=dx;
            yhtemp+=slopeh*dx*TMath::Sin(phih);
            zhtemp+=slopeh*dx*TMath::Cos(phih);
            yh=rnd->Gaus(yhtemp,3);
            zh=rnd->Gaus(zhtemp,3);
