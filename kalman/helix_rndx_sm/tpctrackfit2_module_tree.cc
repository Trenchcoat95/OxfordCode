...
namespace gar {
  namespace rec {
      ...
      else
	{
      ...
      TTree t1_BAK("t1_BAK","Backward fitter tree");
      t1_BAK.Branch("xht",&xht,"xht/F");
      t1_BAK.Branch("yht",&yht,"yht/F");
      t1_BAK.Branch("zht",&zht,"zht/F");
      t1_BAK.Branch("xpost",&xpost,"xpost/F");
      t1_BAK.Branch("ev",&ev,"ev/I");
      t1_BAK.Branch("parvect",&parvect);
      t1_BAK.Branch("predstept",&predstept);
      t1_BAK.Branch("Pt",&Pt);
                                ...
                                )
    {
     ...
     TMatrixF P(5,5);  // covariance matrix of parameters
      ...
      //TFile f("/dune/data/users/battisti/thinecaltest/Energy_samples/demo.root","recreate");
      ...
      for (size_t iTPCCluster=1; iTPCCluster<nTPCClusters; ++iTPCCluster)
        {
          ...
          Zh->SetPoint(iTPCCluster-1,xh,zh);
          YZh->SetPoint(iTPCCluster-1,yh,zh);
          Yh->SetPointError(iTPCCluster-1,0,TMath::Sqrt(R[0][0]));
          Zh->SetPointError(iTPCCluster-1,0,TMath::Sqrt(R[1][1]));
          YZh->SetPointError(iTPCCluster-1,TMath::Sqrt(R[0][0]),TMath::Sqrt(R[1][1]));
          */
          if (fPrintLevel > 0)
            {
              std::cout << std::endl;
          ...
          if (dx == 0) dx = 1E-3;
          std::cout << "dxdenom, dxnum,slope: " << dxdenom << " " << dxnum <<" "<<slope<< std::endl;
          std::cout<<"dx-dxpart "<<((dxnum-(xh - xpos)/(fTPCClusterResolX*fTPCClusterResolX))/dxdenom)<<" dx-dyzpart "<<((xh - xpos)/(fTPCClusterResolX*fTPCClusterResolX))/dxdenom<<std::endl;
          std::cout << "Sigmax: " << fTPCClusterResolX << "  Sigmayz " << fTPCClusterResolYZ << std::endl;
