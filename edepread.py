...
def loop( events, tgeo, tout, nfiles, okruns ):
	...
	# loop over the primary vertices
        for ivtx,vertex in enumerate(event.Primaries):
            #print (dir(vertex))
            node_all_prim = tgeo.FindNode(vertex.GetPosition().X(), vertex.GetPosition().Y(),vertex.GetPosition().Z())
  	    ...
  	    # initialize the tree variables
            ...
            for i in range(3):
                t_vtx_no_off[i] = vertex.GetPosition()[i] / 10.
	    ...
	    # loop over the primary particles
            for ipart,particle in enumerate(vertex.Particles):
                ...
                t_m_recon_gen[0] = m
                t_p_true_gen[0] = p
                t_pdg_gen[0] = particle.GetPDGCode()
                ...
                if abs(particle.GetPDGCode()) in [11,12,13,14]:
                    ileptraj = particle.GetTrackId()
                    t_lepPdg[0] = particle.GetPDGCode()
		...
		# record which detector volume does the muon end up in
            ...
            for hit in hits:
                hStart = ROOT.TVector3( hit.GetStart()[0]/10.-offset[0], hit.GetStart()[1]/10.-offset[1], hit.GetStart()[2]/10.-offset[2] )
                hStop = ROOT.TVector3( hit.GetStop()[0]/10.-offset[0], hit.GetStop()[1]/10.-offset[1], hit.GetStop()[2]/10.-offset[2] )
                hStartYZ = ROOT.TVector3( 0., hit.GetStart()[1]/10.-offset[1], hit.GetStart()[2]/10.-offset[2] )
                hStopYZ = ROOT.TVector3( 0., hit.GetStop()[1]/10.-offset[1], hit.GetStop()[2]/10.-offset[2] )
                if event.Trajectories[hit.GetPrimaryId()].GetParentId() == -1:
                    track_length[fsParticleIdx[hit.GetPrimaryId()]] += (hStop-hStart).Mag()
                    track_length_perp[fsParticleIdx[hit.GetPrimaryId()]] += (hStopYZ-hStartYZ).Mag()
                # below is to be ignored for gas TPC
                if hit.GetPrimaryId() != ileptraj:
                    hStart = ROOT.TVector3( hit.GetStart()[0]/10.-offset[0], hit.GetStart()[1]/10.-offset[1], hit.GetStart()[2]/10.-offset[2] )
...
if __name__ == "__main__":
    ...
    parser.add_option('--first_run', type=int, help='First run number', default=251)
    parser.add_option('--last_run', type=int, help='Last run number', default=251)
    ...
    for run in range( args.first_run, args.last_run+1 ):
        if args.grid:
            ...
            fname = "%sLAr.%s.%d.edepsim.root" % (args.topdir, neutrino, run)
        print (fname)
